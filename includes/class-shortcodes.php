...
class EDDCDP_Shortcodes {
    ...
    public function maybe_override_edd_shortcodes() {
        ...
        if (!empty($original_shortcodes)) {
            wp_cache_set('eddcdp_original_shortcodes', $original_shortcodes, 'eddcdp', HOUR_IN_SECONDS);
        }
    }
    ...
    public function dashboard_shortcode($atts, $content = '', $tag = '') {
        ...
        if ($this->rendered) {
            return '<div class="eddcdp-notice">' . __('Dashboard can only be displayed once per page.', 'eddcdp') . '</div>';
        }
    ...
    private function render_login_form() {
        ...
        <div class="eddcdp-login-required">
            <div class="eddcdp-login-box">
                <h3><?php _e('Customer Dashboard', 'eddcdp'); ?></h3>
                ...
                if (function_exists('wp_login_form')) {
                    ...
                    echo '<p><a href="' . esc_url($login_url) . '" class="eddcdp-login-link">' . __('Log In', 'eddcdp') . '</a></p>';
                }
                ?>
                ...
                <div class="eddcdp-login-links">
                    <?php if (get_option('users_can_register')) : ?>
                        <p>
                            <a href="<?php echo esc_url(wp_registration_url()); ?>">
                                <?php _e('Create an account', 'eddcdp'); ?>
                            </a>
                        </p>
                    <?php endif; ?>
                    ...
                    <p>
                        <a href="<?php echo esc_url(wp_lostpassword_url(get_permalink())); ?>">
                            <?php _e('Lost your password?', 'eddcdp'); ?>
                        </a>
                    </p>
                </div>
            </div>
    ...
    private function render_access_denied() {
        return '<div class="eddcdp-access-denied">' . 
               '<p>' . __('You do not have permission to access this dashboard.', 'eddcdp') . '</p>' .
               '</div>';
    }
    ...
    private function render_template_error() {
        if (current_user_can('manage_options')) {
            return '<div class="eddcdp-error">' .
                   '<p>' . __('Dashboard template could not be loaded. Please check your template settings.', 'eddcdp') . '</p>' .
                   '<p><a href="' . admin_url('edit.php?post_type=download&page=eddcdp-settings') . '">' . __('Go to Settings', 'eddcdp') . '</a></p>' .
                   '</div>';
        }
        
        return '<div class="eddcdp-error">' .
               '<p>' . __('Dashboard is temporarily unavailable. Please try again later.', 'eddcdp') . '</p>' .
               '</div>';
    }
    
    /**
     * Reset rendered state (useful for testing)
     */
    public function reset_rendered_state() {
        $this->rendered = false;
    }
    
    /**
     * Check if dashboard was already rendered
     */
