...
if (!$dashboard_data->is_wishlist_active()) {
    ...
    <div class="eddcdp-empty-state">
        ...
        <h3><?php _e('Wishlist not available', EDDCDP_TEXT_DOMAIN); ?></h3>
        <p><?php _e('Wishlist functionality requires the Wish Lists add-on.', EDDCDP_TEXT_DOMAIN); ?></p>
    </div>
    <?php
    return;
}
...
<h2 class="eddcdp-section-title"><?php _e('Your Wishlist', EDDCDP_TEXT_DOMAIN); ?></h2>
        ...
        <?php foreach ($wishlist_items as $item) : ?>
            ...
            <div class="eddcdp-wishlist-item">
                ...
                <?php if (edd_has_variable_prices($download->ID)) : ?>
                    <p><?php _e('Variable pricing', EDDCDP_TEXT_DOMAIN); ?></p>
                <?php else : ?>
...
<?php else : ?>
    <div class="eddcdp-empty-state">
        <div class="eddcdp-empty-icon">❤️</div>
        <h3><?php _e('Your wishlist is empty', EDDCDP_TEXT_DOMAIN); ?></h3>
        <p><?php _e('Save items for later purchase by adding them to your wishlist.', EDDCDP_TEXT_DOMAIN); ?></p>
        <a href="<?php echo home_url(); ?>" class="eddcdp-btn"><?php _e('Browse Products', EDDCDP_TEXT_DOMAIN); ?></a>
    </div>
<?php endif; ?>